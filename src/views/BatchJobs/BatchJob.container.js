...
class Order extends Component {
    ...
    _renderDeliveredData() {
        const { data, classes } = this.props;
        const temp = [];
        if ([Constants.JOB_STATUS.DELIVERED, Constants.JOB_STATUS.UNDELIVERED].indexOf(data.status) >= 0) {
            if (data.status == Constants.JOB_STATUS.DELIVERED) {
                temp.push(<TableRow>
            ...
            temp.push(<>
                <TableRow>
                    <TableCell classes={{root: classes.tableCell}}>Updated At</TableCell>
                    <TableCell classes={{root: classes.tableCell}}>
                        <a href={"https://www.google.com/maps/search/?api=1&query="+data.delivery_loc.coordinates[1]+','+data.delivery_loc.coordinates[0]} target={'_blank'}>
                            Location
                        </a>
                    </TableCell>
                </TableRow>
                <TableRow>
                    <TableCell classes={{root: classes.tableCell}}>Location Image</TableCell>
                    <TableCell classes={{root: classes.tableCell}}>
                        <a href={data.location_image} target={'_blank'}><img src={data.location_image} style={{ width: '100px' }} alt=""/></a>
                    </TableCell>
                </TableRow>
            </>);
