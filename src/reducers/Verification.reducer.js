...
export default function (state = JSON.parse(JSON.stringify(initialState)), action) {
    switch (action.type) {
        ...
        case ASSIGN_DRIVER_TO_VERIFICATION: {
            if (action.payload) {
                let tempIndex = [];
                const prevState = state.all;
                prevState.forEach((val, index) => {
                    if (action.payload.selection.indexOf(val.id) >= 0) {
                        ...
                        val.is_verifying = true;
                        val.driver_name = action.payload.driver_name;
                        val.driver_contact = action.payload.driver_contact;
                    }
                });
